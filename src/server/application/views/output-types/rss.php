<?php
    header("Content-Type: application/rss+xml; charset=utf-8");

    $rssfeed = '<?xml version="1.0" encoding="utf-8"?>'."\r\n";
    $rssfeed .= '<rss version="2.0" xmlns:atom="http://www.w3.org/2005/Atom">'."\r\n";
    $rssfeed .= '<channel>'."\r\n";
    $rssfeed .= '<title>Gaunt Face | Matt Gaunt</title>'."\r\n";
    $rssfeed .= '<link>'.base_url().'</link>'."\r\n";
    $rssfeed .= '<description>I\'m Matt Gaunt, a developer currently at Google. Here are my ramblings.</description>'."\r\n";
    $rssfeed .= '<language>en-uk</language>'."\r\n";
    $rssfeed .= '<copyright>Copyright (C) '.date("Y").' gauntface.com</copyright>'."\r\n";
    $rssfeed .= '<atom:link href="'.htmlspecialchars(current_url()).'" rel="self" type="application/rss+xml" />'."\r\n";
    for($i = 0; $i < count($posts); $i++) {
        $post = $posts[$i];
        $rssfeed .= '<item>'."\r\n";
        $rssfeed .= '<title><![CDATA['.$post->getTitle().']]></title>'."\r\n";
        $rssfeed .= '<description><![CDATA['.$post->getExcerptHTML().']]></description>'."\r\n";

        $itemUrl = rtrim(base_url(), "/").$post->getPublicURL();

        $rssfeed .= '<link>' . $itemUrl . '</link>'."\r\n";
        $rssfeed .= '<guid isPermaLink="false">' . $itemUrl . '</guid>'."\r\n";

        $rssfeed .= '<pubDate>' . date("D, d M Y H:i:s O", $post->getPublishTime()) . '</pubDate>'."\r\n";
        $rssfeed .= '</item>'."\r\n";
    }
    $rssfeed .= '</channel>'."\r\n";
    $rssfeed .= '</rss>';

    echo $rssfeed;
?>
